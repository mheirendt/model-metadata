...
/**
 * **Check to see if the value on the left is *greater than* the value on the right**<br />
 ...
 * const operator = OperatorMap.get('gt');
 * 
 * const a = operator.execute(5, 'less'); // => true
 * const b = operator.execute(5, [0, 1, 2, 3, 4, 5, 6]); // => false
 * 
 * // OR
 * 
 * const c = operator.execute(new Date('01/01/2000'), new Date('01/01/2020')); // => false
 * const d = operator.execute(new Date('01/01/2020'), new Date('01/01/2000')); //=> true 
 * ```
 */
@injectable()
export class GreaterThan extends OperatorBase {
